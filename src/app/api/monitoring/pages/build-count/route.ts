...
export async function GET(request: Request): Promise<Response> {
  ...
  if (q instanceof Response)
    return q
  try {
    const { describePagesBuildCount, resolvePagesZoneId } = await import('@/server/monitoring')
    const zoneId = await resolvePagesZoneId(cred.secretId, cred.secretKey, q.zoneId)
    if (!zoneId)
      return fail(400, '缺少 ZoneId 且无法自动解析', 400)
    const data = await describePagesBuildCount(cred.secretId, cred.secretKey, zoneId)
    return ok(data)
  }
  catch (err) {
